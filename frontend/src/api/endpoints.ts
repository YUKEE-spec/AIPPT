...
import { getApiConfigForRequest } from '@/utils/api-config';
import { recordApiUsage } from '@/utils/api-stats';
...
export const generateOutline = async (projectId: string): Promise<ApiResponse> => {
  ...
  try {
    ...
    if (defaultTextApi) {
      recordApiUsage(defaultTextApi.provider, 'text', true, responseTime);
    }
    ...
    const defaultTextApi = store.getDefaultTextApi();
    if (defaultTextApi) {
      recordApiUsage(defaultTextApi.provider, 'text', false, responseTime);
    }
    throw error;
  }
};
...
export const generateFromDescription = async (projectId: string, descriptionText?: string): Promise<ApiResponse> => {
  const apiConfig = getApiConfigForRequest();
  const requestData = descriptionText ? { description_text: descriptionText } : {};
...
export const generatePageImage = async (
  ...
  const response = await apiClient.post<ApiResponse>(
    ...
    { force_regenerate: forceRegenerate, ...apiConfig }
  );
  return response.data;
};
...
export const editPageImage = async (
  ...
  if (contextImages?.uploadedFiles && contextImages.uploadedFiles.length > 0) {
    ...
    if (Object.keys(apiConfig).length > 0) {
      formData.append('api_config', JSON.stringify(apiConfig.api_config));
    }
    // 添加上传的文件
